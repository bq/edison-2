...
#endif  
#endif
void hdmi_spk_work(struct work_struct *work)
{
#ifdef CONFIG_RK_HDMI_CTL_CODEC
    if(HDMI_HPD_REMOVED==hdmi->hotplug)
    #ifdef CONFIG_MACH_RK_FAC
	#ifdef CONFIG_SND_RK29_SOC_ES8323
...
void hdmi_sys_remove(void)
{
	...
	#endif
	#ifdef CONFIG_RK_HDMI_CTL_CODEC
	cancel_delayed_work_sync(&(hdmi->hdmi_spk_delay_work));
	queue_delayed_work(hdmi->workqueue,&(hdmi->hdmi_spk_delay_work),150);
#ifdef CONFIG_RK_HDMI_CTL_CODEC
#ifdef CONFIG_MACH_RK_FAC
	#ifdef CONFIG_SND_RK29_SOC_ES8323
		es8323_codec_set_spk(1);
