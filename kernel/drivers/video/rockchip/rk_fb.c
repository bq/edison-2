	...
	struct rk_lcdc_device_driver *def_drv,int id)
{
    ...
    if(dev_drv->screen_ctr_info->prop == PRMRY) //show logo for primary display device
    {
		...
		if(fb_prepare_logo(fb_inf->fb[0], FB_ROTATE_UR)) {
			/* Start display and show logo on boot */
			fb_set_cmap(&fb_inf->fb[0]->cmap, fb_inf->fb[0]);
#if defined(CONFIG_POWER_ON_CHARGER_DISPLAY) && defined(CONFIG_BATTERY_RK30_ADC_FAC)
			if((rk_get_system_battery_status() == POWER_SUPPLY_TYPE_BATTERY) ||
				(board_boot_mode() == BOOT_MODE_REBOOT) || (board_boot_mode() == BOOT_MODE_RECOVERY))
#ifdef CONFIG_MALATA_D7803_Q7
                       fb_show_logo(fb_inf->fb[0], FB_ROTATE_CW);
#else
						fb_show_logo(fb_inf->fb[0], FB_ROTATE_UR);
#endif
#elif defined(CONFIG_POWER_ON_CHARGER_DISPLAY) && defined(CONFIG_MFD_RT5025)
		if(pmic_is_rt5025())
			if(!rt5025_cable_exist() || (board_boot_mode() == BOOT_MODE_REBOOT) || (board_boot_mode() == BOOT_MODE_RECOVERY))
				fb_show_logo(fb_inf->fb[0], FB_ROTATE_UR);
#else
			fb_show_logo(fb_inf->fb[0], FB_ROTATE_UR);
#endif
