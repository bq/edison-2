...
int modem_poweron_off(int on_off)
{
	...
	if(on_off)
	{
		gpio_set_value(pdata->modem_power_en, GPIO_HIGH);  //Let 3G_PWR be high first
		msleep(3000);
		//gpio_set_value(pdata->modem_usb_en, !pdata->modem_usb_en_active);
		msleep(100);
		gpio_set_value(pdata->bp_power, GPIO_HIGH);  //Let 3G_EN be high
		//gpio_set_value(pdata->modem_uart_en, pdata->modem_uart_en_active);
		//gpio_set_value(pdata->ap_ready, !pdata->ap_ready_active);
	}
	else
	{
		gpio_set_value(pdata->bp_power, GPIO_LOW);  //Let 3G_EN be low first
		//gpio_set_value(pdata->modem_usb_en, pdata->modem_usb_en_active);
		msleep(3000);
		gpio_set_value(pdata->modem_power_en, GPIO_LOW);  //Let 3G_PWR be low
		//gpio_set_value(pdata->modem_uart_en, !pdata->modem_uart_en_active);
		//gpio_set_value(pdata->ap_ready, pdata->ap_ready_active);
	}
  return 0;
...
static ssize_t mt6229_write(struct file *file, const char __user *buf,size_t len, loff_t *off)
{
	...
	if (cmd[0] == '0')
	{
//		gpio_set_value(gpdata->ap_ready, !gpdata->ap_ready_active);
	}	
	if (cmd[0] == '1')
	{
//		gpio_set_value(gpdata->ap_ready, gpdata->ap_ready_active);
	...
	if (cmd[0] == '2')
	{
//		gpio_set_value(gpdata->modem_uart_en, gpdata->modem_uart_en_active);
	}
	if (cmd[0] == '3')
	{
//		gpio_set_value(gpdata->modem_uart_en, !gpdata->modem_uart_en_active);
	...
	if (cmd[0] == '4')
	{
//		gpio_set_value(gpdata->modem_usb_en, gpdata->modem_usb_en_active);
	}if (cmd[0] == '5')
	{
//		gpio_set_value(gpdata->modem_usb_en, !gpdata->modem_usb_en_active);
	}
...
static void mt6229_poweron_timer(struct work_struct *work)
{
	struct rk29_mt6229_data *pdata = container_of((work),struct rk29_mt6229_data,power_work);
	if(!strstr(saved_command_line,"charger")){
		modem_poweron_off(1);
		modem_status = 1;
	}else
		queue_delayed_work(pdata->wq,&pdata->power_work,msecs_to_jiffies(1000*5));
}
...
static int mt6229_probe(struct platform_device *pdev)
{
	...
	pdata->wq = create_singlethread_workqueue("mt6229_power");
	INIT_DELAYED_WORK(&pdata->power_work, mt6229_poweron_timer);
	//modem_poweron_off(1);
	//modem_status = 1;
	...
	queue_delayed_work(pdata->wq,&pdata->power_work,msecs_to_jiffies(1000*10));
	return result;
err0:
	kfree(mt6229_data);
err1:
