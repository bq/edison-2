...
extern struct wake_lock hid_wake_lock;
extern int hid_wake_lock_status;
...
static int hub_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if(hdev->serial){
		memcmp_result = memcmp(hub_usb_otg, hdev->serial, strlen(hub_usb_otg));
	}
#endif
	...
	if ((!hub_is_superspeed(hdev) && !hdev->parent) && (memcmp_result != 0))
...
void usb_disconnect(struct usb_device **pdev)
{
	...
	if(hid_wake_lock_status == 1){
		wake_unlock(&hid_wake_lock);
		hid_wake_lock_status = 0;
	}
 ...
 SuspendCleared:
	...
	if (status < 0) {
		...
		if(status != -EHOSTUNREACH){
			...
			printk("%s: recieve -113 error, ignore!\n", __func__);
			status = 0;
		}
	}
	return status;
