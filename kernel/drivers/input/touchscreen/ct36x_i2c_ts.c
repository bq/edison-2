...
{
	#if (CT36X_TS_ABS_X_MAX == 1024)
	#include "CT363_xd97xk18_97_V01_121029.dat" 	//1024x768
	#else
	#include "CT363LR2030D_V0A120602A_2048x1536.dat"  //2048x1536
	#endif
};
...
static void ct36x_ts_workfunc(struct work_struct *work)
{
	...
	for ( iter = 0; iter < CT36X_TS_POINT_NUM; iter++ ) {
		...
		//printk("origin x:%d>>y:%d\n",x,y);
		  ...
		  //x=(ts->x_max*x/CT36X_TS_ABS_X_MAX);
			//y=(ts->y_max*y/CT36X_TS_ABS_Y_MAX);
...
static int ct36x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct ct36x_ts_info_platform *ts_platform= client->dev.platform_data;
	struct device *dev;
	int err = -1;
	char fw_ver_upd;
	...
	err = ts_platform->rst;
	printk("%s...ts=%x\n",__func__,ts);
	ts->rst = ts_platform->rst;
	ts->ss = ts_platform->irq;
	...
	ct36x_ts_hw_reset(ts);
      ...
      //check chip
       ts->chip_id = ct36x_ts_get_chip(client);
       printk("------------------------------------ ts->chip_id = %d \n",ts->chip_id);
	 if ( ts->chip_id != CT360_CHIP_VER && ts->chip_id != CT36X_CHIP_VER ) {
		printk(">>>> Read CT363 Chip ID Error (%x), quit CT363 drivers ! \n", ts->chip_id);
	      goto ERR_I2C_CHK;
	 }
	...
	ts->data.buf[2] = 0xFF;
#endif
	ts->x_max = CT36X_TS_ABS_X_MAX;
	ts->y_max = CT36X_TS_ABS_Y_MAX;
	ct36x_ts_reg_write(client, client->addr, (char *) ts->data.buf, 3, CT36X_TS_I2C_SPEED);
	mdelay(10);
...
/*
	memset(&ct36x_ts,0,sizeof(struct ct36x_ts_info));
