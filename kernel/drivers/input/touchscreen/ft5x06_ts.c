...
extern int is_usb_charging(void);
#endif
/***********************************************************************************************
...
static void ft5x0x_report_value(void)
{
	...
	for (i = 0; i < event->touch_point; i++)
	{
		...
		if (event->au8_touch_event[i] == 0 ||
			...
			event->au16_y[i] = SCREEN_MAX_X - event->au16_y[i];
...
#endif
#if CHANGE_THRESHOLD_IN_CHARGE
static int ft5x0x_ts_charger_detect(void)
{
    int ret = -1;
	unsigned char threshold_value;
	static int discharging_touch_threshold = 0;
	static int discharging_release_threshold = 0;

    if (0 == discharging_touch_threshold || 0 == discharging_release_threshold)
    {
        /* read and store TP sensitivity value first, use these values when discharging */
        ret = ft5x0x_read_reg(0x80, &discharging_touch_threshold);
        if (ret < 0)
        {
            printk("ft5x0x_ts: ft5x0x_ts_charger_detect, read register failed.\n");
            return -1;
        }

        ret = ft5x0x_read_reg(0x81, &discharging_release_threshold);
        if (ret < 0)
        {
            printk("ft5x0x_ts: ft5x0x_ts_charger_detect, read register failed.\n");
            return -1;
        ...
        /*printk("YLM==>, charge status is %d. touch_threshold is %d. release_threshold is %d\n", 
            is_usb_charging(), discharging_touch_threshold, discharging_release_threshold);*/
    }
    else
    {
        ret = ft5x0x_read_reg(0x80, &threshold_value);
        //printk("YLM==>, charge status is %d. threshold_value is %d.\n", is_usb_charging(), threshold_value);
        ...
        {
            /* DC IN status */
            if(1 == is_usb_charging())
            {
                /* change sensitivity , threshold value is 35 x 4 , peak value is 100*/
                if(threshold_value != CHARGING_TOUCH_THRESHOLD)
                {
                    ft5x0x_write_reg(0x80,CHARGING_TOUCH_THRESHOLD);
                    ft5x0x_write_reg(0x81,CHARGING_RELEASE_THRESHOLD);
                }
            }
            else  /*Not DC in */
            {
                /* change sensitivity , threshold value is 20 x 4 , peak value is 60 */
                if(threshold_value != discharging_touch_threshold)
                {
                    ft5x0x_write_reg(0x80,discharging_touch_threshold);
                    ft5x0x_write_reg(0x81,discharging_release_threshold);
                }
            }
        }
	}
...
ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
        ...
        if( uc_reg_value != 0xff )
	{
          ...
          #if  defined(CONFIG_MALATA_D1004)||defined(CONFIG_MALATA_D1025)||defined(CONFIG_MALATA_D1024)
               /* for xiamen tianzhiyu TP , the firmare is 0xax to 0xbx */
 		if(( uc_reg_value < 0xc1) && ( uc_reg_value > 0xa0))
	...
	#endif
printk("YLM==>probe, screen x = %d. screen y = %d.\n", SCREEN_MAX_X, SCREEN_MAX_Y);
#ifdef CONFIG_FT5X0X_MULTITOUCH
	#ifdef NEW_MT_SLOT_REPORT
