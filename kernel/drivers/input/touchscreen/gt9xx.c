...
static void goodix_ts_work_func(struct work_struct *work)
{
...
#if defined(CONFIG_MALATA_D7005)||defined(CONFIG_MALATA_D7005A)||defined(CONFIG_MALATA_D1024)
    /* if data is not ready , delay 1 ms and read point data again */    
    if((point_data[GTP_ADDR_LENGTH] == 0)||(point_data[GTP_ADDR_LENGTH] == 0x80))
    {
    	msleep(1);	
        ret = gtp_i2c_read(ts->client, point_data, 12);
        if (ret < 0)
    ...
    GTP_DEBUG("pre_touch:%02x, finger:%02x.\n", pre_touch, finger);
...
static irqreturn_t goodix_ts_irq_handler(int irq, void *dev_id)
{
	...
	int io = gpio_get_value(i2c_connect_client->irq);
	//printk("goodix_ts_irq_handler, io:%d ts->int_trigger_type %d\n", io,ts->int_trigger_type);
    GTP_DEBUG_FUNC();
	if(ts->int_trigger_type==1){//IRQ_TYPE_EDGE_FALLING
		if (io == 0){
		    gtp_irq_disable(ts);
		    queue_work(goodix_wq, &ts->work);
		}
	}else if(ts->int_trigger_type==0){//IRQ_TYPE_EDGE_RISING
		if (io == 1){
		    gtp_irq_disable(ts);
		    queue_work(goodix_wq, &ts->work);
		}
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    { 	
	 ...
	 msleep(50);
        ...
        /* IF SENSOR ID  is  1 , then use default config */
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   /* ret = gtp_request_io_port(ts);
    ...
    ret=gpio_request(gt9110_int_number, "GT9110_IRQ_PIN");
    if (ret != 0) {
	 gpio_free(gt9110_int_number);
	 GTP_ERROR("GT9110 gpio_request GT9110_IRQ_PIN error\n");
	 return ret;
    }


    ret = gpio_request(gt9110_reset_number, "GT9110_RESET_PIN");
    if (ret != 0) {
	 gpio_free(gt9110_reset_number);
	 GTP_ERROR("GT9110 gpio_request gt9110_reset_number error\n");
	 return ret;
   }
