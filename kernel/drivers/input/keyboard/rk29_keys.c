...
static irqreturn_t keys_isr(int irq, void *dev_id)
{
	...
	if(strcmp(button->desc,"hall") == 0){
		if(gpio_get_value(button->gpio) != button->active_low)
		{
			input_event(input, type, button->code, 1);
			input_sync(input);
			input_event(input, type, button->code, 0);
			input_sync(input);
			key_dbg(bdata, "%skey[%s]: report ev[%d]\n",
				(button->gpio == INVALID_GPIO)?"ad":"io", button->desc, button->code);
		}else{
			input_event(input, type, button->code1, 1);
			input_sync(input);
			input_event(input, type, button->code1, 0);
			input_sync(input);
			key_dbg(bdata, "%skey[%s]: report ev[%d]\n",
				(button->gpio == INVALID_GPIO)?"ad":"io", button->desc, button->code1);
		}
...
static int __devinit keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if(button->code1)
			input_set_capability(input, EV_KEY, button->code1);
	};
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if(button->gpio != INVALID_GPIO) {
			...
			if(strcmp(button->desc,"hall") == 0)
				gpio_pull_updown(button->gpio, GPIOPullUp);
			#endif
