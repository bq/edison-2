				...
				struct lis3de_acc_status *stat, int *xyz)
{
	...
	hw_d[0] = (s16) ((s8) acc_data[1]);
	hw_d[1] = (s16) ((s8) acc_data[3]);
	hw_d[2] = (s16) ((s8) acc_data[5]);
...
static void lis3de_acc_input_work_func(struct work_struct *work)
{
	...
	int xyz[3] = { 0 };
	int err;
	printk("lis3de_acc_input_work_func\n");
		...
		const struct i2c_device_id *id)
{
	...
	if (client->dev.platform_data == NULL) {
		...
		printk("yemk:client->dev.platform_data != NULL\n");
...
*/
	...
	printk("yemk:lis3de_acc_probe end\n");
	return 0;
