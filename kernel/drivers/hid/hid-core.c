...
struct wake_lock hid_wake_lock;
int hid_wake_lock_status = 0;
/*
...
int hid_connect(struct hid_device *hdev, unsigned int connect_mask)
{
	...
	if(hdev->bus == BUS_USB)
		if((strcmp(type,"Mouse") == 0)||(strcmp(type,"Keyboard") == 0)||(strcmp(type,"Keypad") == 0))
			if(!hid_wake_lock_status){
				wake_lock(&hid_wake_lock);
				hid_wake_lock_status = 1;
			}
...
static int __init hid_init(void)
{
	...
	hid_debug_init();
	wake_lock_init(&hid_wake_lock, WAKE_LOCK_SUSPEND, "hid_lock");
