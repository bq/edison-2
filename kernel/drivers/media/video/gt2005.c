...
static u16 DGain_shutter,AGain_shutter,DGain_shutterH,DGain_shutterL,AGain_shutterH,AGain_shutterL,shutterH,shutterL,shutter;
static u16 UXGA_Cap = 0;
...
static int sensor_s_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
    ...
    if ((int)winseqe_set_addr  != sensor->info_priv.winseqe_cur_addr) {
		...
		if (winseqe_set_addr == sensor_uxga) { //james
			...
			AGain_shutter = AGain_shutter >> 1;
			AGain_shutterH= AGain_shutter >> 8;
			AGain_shutterL = AGain_shutter&0XFF;
	...
	if(flash_on_off == 1)
	{
		...
		hrtimer_start(&(flash_off_timer.timer),ktime_set(0, 100*1000*1200),HRTIMER_MODE_REL);
	}
	mf->width = set_w;
    mf->height = set_h;
	...
	case V4L2_CID_OPEN_FLASH:
		{
		 ...
		 sensor_read(client, 0x0010, &y_value1);
		 sensor_read(client, 0x0011, &y_value2);
		 hight_value =(y_value1&3);
			if((hight_value > 0) || (y_value2 > 120))
		 	ext_ctrl ->value = 0;
		 else 
		 	ext_ctrl->value =1;	
		}
