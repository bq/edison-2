...
#ifdef CONFIG_CHARGE_EMI_ENABLE
	if(bq27541_cap == 100)
		bq24196_set_input(1);
	else
		bq24196_set_input(0);
...
static void bq27541_update_data(void)
{
	...
	buf[1] = 0;
	bq27541_read(di->client,0x00,buf,2);
	printk("check hardware version:buf[0] =0x%x, buf[1] = 0x%x\n", buf[0], buf[1]);
	if(buf[0] == 0x41 && buf[1] == 0x05)
	{
		...
		buf[1] = 0;
		bq27541_read(di->client,0x00,buf,2);
		printk("check software version:buf[0] =0x%x, buf[1] = 0x%x\n", buf[0], buf[1]);
		if(buf[0] == 0x24 && buf[1] == 0x02)
		{
			/*unseal*/
			buf[0] = 0x14;
			buf[1] = 0x04;
			...
			buf[1] = 0;
			bq27541_read(di->client,0x00,buf,2);
			printk("check unseal version:buf[0] =0x%x, buf[1] = 0x%x\n", buf[0], buf[1]);
			if(((buf[0]  | buf[1] << 8) & 0x6000) == 0)
			{
				 /*write data*/
				buf[0] = 0x00;
				bq27541_write(di->client,0x61,buf,1);
				msleep(10);

				buf[0] = 0x40;
				buf[1] = 0x00;
				bq27541_write(di->client,0x3e,buf,2);
				msleep(10);

				buf[0] = 0;
				bq27541_read(di->client,0x43,buf,1);
				printk("check reset status:buf[0] =0x%x\n", buf[0]);
				if(buf[0] != 0x08)
				{
					buf[0] = 0x08;
					bq27541_write(di->client,0x43,buf,1);
					msleep(10);

					buf[0] = 0xC3;
					bq27541_write(di->client,0x60,buf,1);
					msleep(10);
					...
					if(((buf[0]  | buf[1] << 8) & 0x6000) == 0)
					{
						/*check data*/
						buf[0] = 0x00;
						bq27541_write(di->client,0x62,buf,1);
						msleep(10);
