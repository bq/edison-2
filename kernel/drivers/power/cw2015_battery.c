...
#define AC_BIG_CHARGE_MODE 1
#define AC_LITTLE_CHARGE_MODE 2
#define NUM_USBCHARGE_IDENTIFY_TIMES	1
#define SYSTEM_SHUTDOWN_VOLTAGE  3500000        //set system shutdown voltage related in battery info.
#define	BATT_USB_CHARGE_LOW_THRES		3550000
#define	BATT_USB_CHARGE_LOW_CHARGER		3600000
...
static int no_standard_ac_charge_mode = 0;
#ifdef CONFIG_DISABLE_PC_CHARGING_DISPLAY
static int pc_charge_display_flag = 0;
#endif
...
struct cw_battery {
		...
		struct delayed_work battery_check_work;
...
static struct wake_lock cw_wake_lock;
...
static int cw_init(struct cw_battery *cw_bat)
{
        ...
        for (i = 0; i < 30; i++) {
                ...
                msleep(100);
                if (i > 25)
                        printk("cw2015/cw2013 input unvalid power error\n");
...
static int cw_get_capacity(struct cw_battery *cw_bat)
{
	...
	cw_capacity = min(cw_capacity, 100);
	if ((cw_bat != NULL) && cw_bat->plat_data->capacity_max && cw_bat->plat_data->capacity_min) {
		cw_capacity = max(cw_capacity, cw_bat->plat_data->capacity_min);
		cw_capacity = ((cw_capacity - cw_bat->plat_data->capacity_min) * 100 +
			(cw_bat->plat_data->capacity_max - cw_bat->plat_data->capacity_min) / 2)
			/ (cw_bat->plat_data->capacity_max - cw_bat->plat_data->capacity_min);
	}
...
static void rk_bat_update_status(struct cw_battery *cw_bat)
{
		...
		DBG("cw_bat->status = %d, cw_bat->charger_mode = %d\n", cw_bat->status, cw_bat->charger_mode);
}
...
#if defined(CONFIG_NO_STANDARD_AC_BIG_CHARGE_MODE)
        if (cw_bat->usb_online == 1) {
                charge_time = time_from_boot - cw_bat->sleep_time_charge_start - cw_bat->run_time_charge_start;
                if (charge_time >= NUM_USBCHARGE_IDENTIFY_TIMES) {
                        if (gadget_status == 0 && dwc_vbus_status() == 1) {
                                usb_status = 2;
					no_standard_ac_charge_mode = AC_BIG_CHARGE_MODE;
                        }else
					no_standard_ac_charge_mode = 0;
                }
        }
#elif defined(CONFIG_NO_STANDARD_AC_LITTLE_CHARGE_MODE)
	if (cw_bat->usb_online == 1) {
		charge_time = time_from_boot - cw_bat->sleep_time_charge_start - cw_bat->run_time_charge_start;
		if (charge_time >= NUM_USBCHARGE_IDENTIFY_TIMES) {
				if (gadget_status == 0 && dwc_vbus_status() == 1) {
					usb_status = 1;
					no_standard_ac_charge_mode = AC_LITTLE_CHARGE_MODE;
				}else
					no_standard_ac_charge_mode = 0;
		}
}
		...
		DBG("%s:usb_status=[%d],cw_bat->charger_mode=[%d],cw_bat->gadget_status=[%d], cw_bat->charger_init_mode = [%d]\n",__func__,usb_status,cw_bat->charger_mode,gadget_status, cw_bat->charger_init_mode);
...
static int rk_usb_update_online(struct cw_battery *cw_bat)
{
...
#ifdef CONFIG_DISABLE_PC_CHARGING_DISPLAY
			if(no_standard_ac_charge_mode == AC_LITTLE_CHARGE_MODE){
		...
		DBG("%s:usb_status =%d, cw_bat->usb_online = %d, cw_bat->dc_online = %d, cw_bat->charger_mode = %d\n", __func__, usb_status, cw_bat->usb_online, cw_bat->dc_online, cw_bat->charger_mode);
...
static void cw_bat_work(struct work_struct *work)
{
		...
		if(cw_bat->plat_data->charge_current_set)
		{
			if(cw_bat->charger_mode == USB_CHARGER_MODE)
				cw_bat->plat_data->charge_current_set(0);
			else if(cw_bat->charger_mode == AC_CHARGER_MODE)
				cw_bat->plat_data->charge_current_set(1);
#ifdef CONFIG_DISABLE_PC_CHARGING_DISPLAY
			else if(cw_bat->charger_mode == 0 && pc_charge_display_flag == 1)
				cw_bat->plat_data->charge_current_set(0);
#endif
		}
		...
		if(cw_bat->charger_mode != 0)
		{
			if(0 == cw_bat->status_lock){
				if (!strstr(saved_command_line,"charger"))
					wake_lock(&cw_wake_lock);
				cw_bat->status_lock = 1;
			}
		}else{
			if(1 == cw_bat->status_lock){
				if (!strstr(saved_command_line,"charger"))
					wake_unlock(&cw_wake_lock);
				cw_bat->status_lock = 0;
			}
		}

        DBG("%s:cw_bat->bat_change = %d, cw_bat->time_to_empty = %d, cw_bat->capacity = %d, cw_bat->voltage = %d, cw_bat->dc_online = %d, cw_bat->usb_online = %d\n",\
                        __func__, cw_bat->bat_change, cw_bat->time_to_empty, cw_bat->capacity, cw_bat->voltage, cw_bat->dc_online, cw_bat->usb_online);
...
#endif
static void cw_bat_power_on_check(struct work_struct *work)
{
	struct delayed_work *delay_work;
	struct cw_battery *cw_bat;
	int ret;
	printk("%s\n", __func__);
	delay_work = container_of(work, struct delayed_work, work);
	cw_bat = container_of(delay_work, struct cw_battery, battery_check_work);

	do{
		rk_ac_update_online(cw_bat);
		rk_usb_update_online(cw_bat);
		rk_bat_update_capacity(cw_bat);
		rk_bat_update_vol(cw_bat);
		DBG("%s :battery voltage = %d, charge_mode = %d\n",__FUNCTION__,cw_bat->voltage,cw_bat->charger_mode);
	}while((cw_bat->voltage <= BATT_USB_CHARGE_LOW_THRES) && (cw_bat->charger_mode == USB_CHARGER_MODE));

	if(cw_bat->charger_mode == USB_CHARGER_MODE){
		if((cw_bat->voltage <= BATT_USB_CHARGE_LOW_CHARGER))
			low_usb_charge = true;
	}else if(cw_bat->charger_mode == 0){
		if((cw_bat->capacity <= 0) || (cw_bat->voltage <= SYSTEM_SHUTDOWN_VOLTAGE)){
			printk("%s :low power and shutdown now!\n",__FUNCTION__);
			kernel_power_off();
		}
	}
}
...
static int cw_bat_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
        ...
        cw_update_time_member_capacity_change(cw_bat);
        cw_update_time_member_charge_start(cw_bat);
		wake_lock_init(&cw_wake_lock, WAKE_LOCK_SUSPEND, "batt_lock");

        cw_bat->battery_workqueue = create_singlethread_workqueue("rk_battery");
        INIT_DELAYED_WORK(&cw_bat->battery_delay_work, cw_bat_work);
        INIT_DELAYED_WORK(&cw_bat->dc_wakeup_work, dc_detect_do_wakeup);
		INIT_DELAYED_WORK(&cw_bat->battery_check_work, cw_bat_power_on_check);
        queue_delayed_work(cw_bat->battery_workqueue, &cw_bat->battery_delay_work, msecs_to_jiffies(10));
        queue_delayed_work(cw_bat->battery_workqueue, &cw_bat->battery_check_work, msecs_to_jiffies(1000));
        if (cw_bat->plat_data->dc_det_pin != INVALID_GPIO) {
                irq = gpio_to_irq(cw_bat->plat_data->dc_det_pin);
