...
//			wake_lock_timeout(&info->usb_wake_lock, 10*HZ);
	...
	if(new_usbval)
	{
		if(info->status_lock == 0)
		{
			wake_lock(&info->usb_wake_lock);
			info->status_lock = 1;
		}
	}else{
		if(info->status_lock == 1)
		{
			wake_unlock(&info->usb_wake_lock);
			info->status_lock = 0;
		}
	}
...
static void usb_detect_work_func(struct work_struct *work)
{
	...
	{
		RTINFO("%s: usb_cnt %d, dwc_vbus_status = %d\n", __func__, pi->usb_cnt, dwc_vbus_status());
/*
		if (strstr(saved_command_line,"charger") && (pi->init_once == 0))
		{
			wake_lock_timeout(&pi->usb_wake_lock, 15*HZ);
			pi->init_once ++;
		}
*/
