...
int bq24196_set_input(int on)
{
	if(bq24196_pdata->input_en_pin != INVALID_GPIO)
	{
		if(on != gpio_get_value(bq24196_pdata->input_en_pin))
			gpio_direction_output(bq24196_pdata->input_en_pin, on);
	}
	return 0;
}	
EXPORT_SYMBOL_GPL(bq24196_set_input);
...
static int bq24196_charge_mode_config(int on)
{
	...
	if(1 == on)
	{
		...
		if(bq24196_pdata->otg_en_pin != INVALID_GPIO)
				 ...
				 const struct i2c_device_id *id)
{
	...
	if(pdata->chg_en_pin != INVALID_GPIO)
	{
		ret = gpio_request(pdata->chg_en_pin, "charge_en");
		if (ret) {
			printk("failed to request charge_en gpio\n");
			return;
		}

		gpio_pull_updown(pdata->chg_en_pin, 0);
		ret = gpio_direction_output(pdata->chg_en_pin, 0);
		if (ret) {
			printk("failed to set gpio charge_en output\n");
		}
		gpio_free(pdata->chg_en_pin);
	}

	if(pdata->input_en_init)
		pdata->input_en_init();
	...
	if(pdata->otg_en_pin != INVALID_GPIO)
	{
...
void bq24196_battery_shutdown(struct i2c_client *client)
{
#ifdef CONFIG_BATTERY_BQ24196_OTG_MODE
