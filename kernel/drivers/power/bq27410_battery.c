...
struct i2c_client *g_bq27410_client;
...
#if 0
		mutex_lock(&g_bq27410_mutex);
		bq27410_write_batt_insert(g_bq27410_client);
		mutex_unlock(&g_bq27410_mutex);
...
#else
		mutex_lock(&g_bq27410_mutex);
		ret = bq27410_read(g_bq27410_client,BQ27410_REG_FLAGS, buf, 2);
		if (ret < 0) {
...
static void bq27410_update_enter_rom_mode(void)
{
	...
	/*X: 1000*/
	g_bq27410_client->addr = 0x0B;//when enter rom mode,the iic addr is 0x0B
	msleep(1000);
}
...
void bq27410_soft_reset(void)
{
	...
	if(g_bq27410_mode == BQ27410_NORMAL_MODE){
		...
		buf[1] = 0x00;
		bq27410_write(g_bq27410_client,0x00,buf,2);
		udelay(66);
	}
}
...
void bq27410_sealed(void)
{
	...
	if(g_bq27410_mode == BQ27410_NORMAL_MODE){
		...
		bq27410_write(g_bq27410_client,0x00,buf,2);
		udelay(66);
		ret = bq27410_read(g_bq27410_client,0x00,buf,2);
	}
}
	...
	if(file_len > 0)
	{
		...
		while(count < file_len)
		{
			...
			if(*pch == 'W')
			{
				...
				//printk("\n");
				//printk("\n W count = %d , wCount=%d\n", count, wCount);
				ret = bq27410_write(g_bq27410_client,buf[1],&buf[2],wCount - 2);
				//printk("W ret =%d ", ret);
				if(ret < 0)
					break;
			}
			...
			else if(*pch == 'C')
			{
				...
				//printk("\n");
				//printk("\n C count = %d , cCount=%d\n", count, cCount);
				ret = bq27410_read(g_bq27410_client,buf[1],&buf[2],cCount - 2);
				//printk("C ret =%d ", ret);
				if(ret < 0)
					break;
			}
...
static int bq27410_update_flash_data(struct file *filp, const char __user *buff, unsigned long len, void *data)
{
	...
	ret = bq27410_read(g_bq27410_client,0x66,buf,2);
	if(ret < 0)
