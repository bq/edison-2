...
extern struct cpufreq_frequency_table dvfs_gpu_table_266[];
extern struct cpufreq_frequency_table dvfs_gpu_table_400[];
extern int gpu_freq_max;
...
static ssize_t gpu_max_freq_store(struct kobject *kobj, struct kobj_attribute *attr,
		const char *buf, size_t n)
{
	if((strncmp(buf, "266", strlen("266")) == 0)){
		dvfs_set_freq_volt_table(clk_get(NULL, "gpu"), dvfs_gpu_table_266);
		gpu_freq_max = 266;
	}else if((strncmp(buf, "400", strlen("400")) == 0)){
		dvfs_set_freq_volt_table(clk_get(NULL, "gpu"), dvfs_gpu_table_400);
		gpu_freq_max = 400;
	}

	return n;
}
static ssize_t gpu_max_freq_show(struct kobject *kobj, struct kobj_attribute *attr,
		char *buf)
{
...
static void gpu_set_freq(void)
{
	char freq[8] = {0};

	long fd = sys_open("/sys/mali400_utility/utility",O_RDWR,0);
...
void gpu_max_freq_set(int value)
{
	if((value == 0) && (gpu_freq_max == 400)){
		dvfs_set_freq_volt_table(clk_get(NULL, "gpu"), dvfs_gpu_table_266);
		gpu_set_freq();
		gpu_freq_max = 266;
	}else if((value == 1) && (gpu_freq_max == 266)){
		dvfs_set_freq_volt_table(clk_get(NULL, "gpu"), dvfs_gpu_table_400);
		gpu_set_freq();
		gpu_freq_max = 400;
	}
	DVFS_DBG("gpu_freq_max = %d\n", gpu_freq_max);
}
EXPORT_SYMBOL(gpu_max_freq_set);
...
static struct dvfs_attribute dvfs_attrs[] = {
	...
	__ATTR(gpu_max_freq,		S_IRWXUGO,	gpu_max_freq_show,	gpu_max_freq_store),
};
