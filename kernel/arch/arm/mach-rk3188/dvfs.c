...
void dvfs_adjust_table_lmtvolt(struct clk *clk, struct cpufreq_frequency_table *table)
{
	...
	if(ifadjust ==0){// for debug,disable leakage referenced dvfs
		printk("--------------------------------adjust volt canceled!\n");
		return;	
	}
	if (leakage_level == 0||leakage_level==15) {
		maxvolt = lkg_volt_table[0].maxvolt;
		printk("--------------------------------if leakage level is 0 or 15,config the maxvolt to 1.35v directly\n");
	}
	else{
