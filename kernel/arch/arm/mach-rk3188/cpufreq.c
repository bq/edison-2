...
#ifdef CONFIG_GPU_FREQ_LIMITED_BY_TEMP
static void gpu_set_freq_400(void)
{
	char freq[8] = {'1'};

	long fd = sys_open("/sys/mali400_utility/malata_threshold",O_RDWR,0);

	if(fd < 0){
		printk("gpu_set_freq: open file failed\n");
		return;
	}
	sys_write(fd,(const char __user *)freq,sizeof(freq));
	sys_close(fd);
	FREQ_DBG("change gpu frequence to 400MHz\n");
}

static void gpu_set_freq_600(void)
{
	char freq[8] = {'0'};
	...
	sys_write(fd,(const char __user *)freq,sizeof(freq));
	sys_close(fd);
	FREQ_DBG("change gpu frequence to 600MHz\n");
}

static void cpu_set_freq_1000(void)
{
	int ret;
	ret = dvfs_set_freq_volt_table(clk_get(NULL, "cpu"), dvfs_arm_table_1000);
	if(!ret)
		FREQ_DBG("change cpu frequence to 1000MHz______________________\n");
	else
		FREQ_DBG("change cpu frequence fail\n");
}

static void cpu_set_freq_1600(void)
{
	int ret;
	ret = dvfs_set_freq_volt_table(clk_get(NULL, "cpu"), dvfs_arm_table_1600);
	if(!ret)
		FREQ_DBG("change cpu frequence to 1600MHz______________________\n");
	else
		FREQ_DBG("change cpu frequence fail\n");
}
...
#ifndef CONFIG_RK3188_STABILITY_PATH
	if (soc_is_rk3188()) {
...
#ifdef CONFIG_RK3188_STABILITY_PATH
	if(0 == rk_leakage_val()){
...
{
#ifdef CONFIG_GPU_FREQ_LIMITED_BY_TEMP
	g_wq = create_singlethread_workqueue("gpu_set");
	INIT_DELAYED_WORK(&delay_work,gpu_max_freq_set_by_temp);
	queue_delayed_work(g_wq,&delay_work,msecs_to_jiffies(30*1000));
#endif
