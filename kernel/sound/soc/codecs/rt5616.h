...
struct rt5616_priv {
	...
	int dmic_en;
	struct rt5616_codec_platform_data *pdata;
	unsigned int spk_ctrl_io;
	bool hdmi_ndet;

	struct timer_list 	jd_timer;
	struct work_struct 	jd_work;
	int last_hp_jd_sts;
	struct switch_dev	headset_sdev;
};
// Add CONFIG_MALATA_D1001 by EvanZeng for adapting the D1001's hardware v1.0
#if defined(CONFIG_MALATA_D7005) || defined(CONFIG_MALATA_D1001) || defined(CONFIG_MALATA_E4201)
#define HP_JD_INSERT	1
#else
#define HP_JD_INSERT	0
#endif

/* Added by LM.Yan. 2013/06/06.
-----------------------------------------------------------------------
	STATE 								Value of ADC
-----------------------------------------------------------------------
BIAS off(not recording mode) 	----	0
BIAS on(recording mode)			----	1023
HP insert with Mic 				----	799
HP insert without Mic 			----	0
-----------------------------------------------------------------------
Below value is 10 times of actual value reported by ADC.
*/
#define ADC_VAL_MIC_HIGH	10230
#define ADC_VAL_MIC_LOW		7000
