...
#ifdef CONFIG_OPEN_MIC_BIAS_DYNAMIC_RK616
extern void mic_det_timer_start(void);
extern void report_mic_status(void);
#endif
...
static int rk616_set_gpio(int gpio, bool level)
{
	...
	if ((gpio & RK616_CODEC_SET_SPK) && rk616 && rk616->spk_ctl_gpio != INVALID_GPIO) {
		...
		if (GPIO_HIGH == level) {
			//set audio pa mode to mode 2, 1.0w
...
#elif defined(CONFIG_MALATA_D1025) || defined(CONFIG_MALATA_D1024) 
		if (GPIO_HIGH == level) {
			//set audio pa mode to mode 3, 0.8w
			gpio_set_value(rk616->spk_ctl_gpio, GPIO_HIGH);
			udelay(8);
			gpio_set_value(rk616->spk_ctl_gpio, GPIO_LOW);
			udelay(8);
...
static struct rk616_reg_val_typ capture_power_up_list[] = {
	...
	{0x84c, 0x3f}, //MIXINL from MIXMUX volume (bit 3-5)
...
#if defined(CONFIG_OPEN_MIC_BIAS_DYNAMIC_RK616) || defined(RK616_HPMIC_FROM_MIC2IN)
			snd_soc_update_bits(rk616->codec, RK616_MICBIAS_CTL,
					RK616_MICBIAS1_PWRD,
					RK616_MICBIAS1_PWRD);
			if(pre_path == Hands_Free_Mic){
				report_mic_status();
			}
#endif
		}
...
{
	if (rk616_for_mid) {
#ifndef CONFIG_OPEN_MIC_BIAS_DYNAMIC_RK616
